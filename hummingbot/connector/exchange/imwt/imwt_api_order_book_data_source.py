...
if TYPE_CHECKING:
    from hummingbot.connector.exchange.imwt.imwt_exchange import IMWTExchange


class IMWTAPIOrderBookDataSource(OrderBookTrackerDataSource):

    _logger: Optional[HummingbotLogger] = None

    def __init__(self,
                 trading_pairs: List[str],
                 connector: 'IMWTExchange',
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        ...
        data = await rest_assistant.execute_request(
            url=web_utils.public_rest_url(path_url=CONSTANTS.IMWT_ORDER_BOOK_PATH.format(pair=pair)),
            params={"limit": 400},
            method=RESTMethod.GET,
            throttler_limit_id=CONSTANTS.IMWT_ORDER_BOOK_PATH,
        )
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        # IMWT never sends diff messages. This method will never be called
        pass
    ...
    def _channel_originating_message(self, event_message: Dict[str, Any]) -> str:
        ...
        if "kind" in event_message:
            ...
            if event_channel == CONSTANTS.IMWT_WS_PUBLIC_TRADES_CHANNEL:
                channel = self._trade_messages_queue_key
            elif event_channel == CONSTANTS.IMWT_WS_PUBLIC_BOOKS_CHANNEL:
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        async with self._api_factory.throttler.execute_task(limit_id=CONSTANTS.WS_CONNECTION_LIMIT_ID):
            await ws.connect(
                ws_url=CONSTANTS.IMWT_WS_URI_PUBLIC,
