...
from hummingbot.connector.exchange.imwt import imwt_constants as CONSTANTS, imwt_web_utils as web_utils
from hummingbot.connector.exchange.imwt.imwt_api_order_book_data_source import IMWTAPIOrderBookDataSource
from hummingbot.connector.exchange.imwt.imwt_api_user_stream_data_source import IMWTAPIUserStreamDataSource
from hummingbot.connector.exchange.imwt.imwt_auth import IMWTAuth
from hummingbot.connector.exchange.imwt.imwt_utils import get_imwt_order_state, to_decimal
...
class ImwtExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self.imwt_api_key = imwt_api_key
        self.imwt_secret_key = imwt_secret_key
    ...
    def authenticator(self):
        return IMWTAuth(
            api_key=self.imwt_api_key,
            secret_key=self.imwt_secret_key,
    ...
    def trading_rules_request_path(self):
        return CONSTANTS.IMWT_INSTRUMENTS_PATH
    ...
    def check_network_request_path(self):
        return CONSTANTS.IMWT_SERVER_TIME_PATH
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return IMWTAPIOrderBookDataSource(
            trading_pairs=self.trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory)

    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return IMWTAPIUserStreamDataSource(
            auth=self._auth,
            connector=self,
            api_factory=self._web_assistants_factory)
    ...
    async def _place_order(self,
        ...
        order_resp = await self._api_request(
            path_url=CONSTANTS.IMWT_PLACE_ORDER_PATH,
            method=RESTMethod.POST,
            data=data,
            is_auth_required=True,
            limit_id=CONSTANTS.IMWT_PLACE_ORDER_PATH,
        )
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        try:
            await self._api_request(
                path_url=CONSTANTS.IMWT_ORDER_CANCEL_PATH.format(order_id=tracked_order.exchange_order_id),
                method=RESTMethod.DELETE,
                is_auth_required=True,
                limit_id=CONSTANTS.IMWT_ORDER_CANCEL_PATH,
            )
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        resp_json = await self._api_request(
            path_url=CONSTANTS.IMWT_TICKER_PATH
        )
    ...
    async def _update_balances(self):
        balances = await self._api_request(
            path_url=CONSTANTS.IMWT_BALANCE_PATH,
            is_auth_required=True)
    ...
    async def _request_order_update(self, order: InFlightOrder) -> Dict[str, Any]:
        ...
        res = await self._api_request(
            ...
            path_url=CONSTANTS.IMWT_ORDER_DETAILS_PATH.format(order_id=order.exchange_order_id),
            is_auth_required=True,
            limit_id=CONSTANTS.IMWT_ORDER_DETAILS_PATH,
        )
        return res
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        new_state = get_imwt_order_state(updated_order_data)
    ...
    async def _user_stream_event_listener(self):
        async for stream_message in self._iter_user_event_queue():
            try:
                ...
                if channel in [CONSTANTS.IMWT_WS_OPENED_ORDERS_CHANNEL, CONSTANTS.IMWT_WS_CLOSED_ORDERS_CHANNEL]:
                    for data in stream_message.get("results", []):
                        order_state = get_imwt_order_state(data)
                ...
                elif channel == CONSTANTS.IMWT_WS_ACCOUNT_CHANNEL:
